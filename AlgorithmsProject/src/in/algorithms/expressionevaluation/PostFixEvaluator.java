...
public class PostFixEvaluator {
	...
	public Integer evaluatePostfixExpression(String postfixExpression) {
		...
		for (int i = 0; i < postfixExpression.length(); i++) {
			...
			if (InfixToPostfix.isOperator(character)) {
				...
				result = performOperation(firstOperand, secondOperand,
						character);
	...
	private Integer performOperation(String firstOperand,
			String secondOperand, Character operator) {
		Integer result = null;
		...
		case '*': return op1 *  op2;
		case '/': return op1 /  op2;
		case '^':  return Integer.getInteger(String.valueOf(Math.pow(op1, op2)));
		default:
			break;
		}
		return result;
